...
var DEFAULT_TIMEOUT = 5; // 10 seconds
...
function inject(bot) {
  ...
  function findPathSync(end, params) {
    ...
    var endRadius =
      params.endRadius == null ? DEFAULT_END_RADIUS : params.endRadius;
    var tooFarThreshold =
      params.tooFarThreshold == null
        ? TOO_FAR_THRESHOLD
        : params.tooFarThreshold;
  ...
  function navigateTo(end, params) {
    ...
    if (results.status == "success") {
      bot.navigate.emit("noPath");
      bot.chat("Found a path, moving now...");
  ...
  function getNeighbors(node) {
    ...
    cardinalDirectionVectors.forEach(function (directionVector) {
      ...
      function pointAt(horizontalOffset, dy) {
        return point.offset(
          directionVector.x * horizontalOffset,
          dy,
          directionVector.z * horizontalOffset
        );
      }
  ...
  function isSafe(block) {
    return (
      block.boundingBox === "empty" && !bot.navigate.blocksToAvoid[block.type]
    );
  }
}
